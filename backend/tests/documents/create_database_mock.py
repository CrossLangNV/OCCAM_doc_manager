...
URL_OVERLAYS = "/documents/overlays/"
...
def create(client=None):
    doc = Document.objects.create(name="Declaration of Independence", content="Stolen by Nicolas Cage.")
    name = "a test image"
    path = "a/b/c.def"
    ...
    with open(FILENAME_IMAGE, "rb") as f:
        files = {"file": f}
        response = client.post(URL_PAGE, data={"document": doc.id, "file": f}, files=files)
    ...
    with open(FILENAME_XML, "r") as f:
        files = {"file": f}
        response = client.post(URL_OVERLAYS, data={"page": page.id, "file": f}, files=files)
